...
//        pthread_cancel(p->socket_read);
        for (i=0; i < p->path_count; i++ ) {
                if (p->mpath[i].keepalive) pthread_kill(p->mpath[i].keepalive_send,SIGUSR1);
                pthread_kill(p->mpath[i].socket_read,SIGUSR1);
                close(p->mpath[i].socket);
        }
        if (p->reorder_window) pthread_kill(p->circlebuffer_handler,SIGUSR1);
        del_routes(p);
        p = p->next;
        if (single_conn) break;
    }
